####### EXPORT INDEPENDENT APPLICATION #######
# pip install pyinstaller
# pyinstaller --windowed --name "BLE_Receiver" bluetooth_finalModule_GUI.py

import sys
import asyncio
import socket  # Added for UDP functionality
...
UDP_IP = "127.0.0.1"
UDP_PORT = 9600
RECONNECT_DELAY = 0  # Reduced delay to speed up scanning
MAX_RETRIES = 3      # Reduced retries to speed up connection
...
class BLEWorker(QThread):
    ...
    async def scan_and_connect(self):
        while not self.stop_flag:
            ...
            if target_device is None:
                self.signals.output.emit(f"{self.target_device_name} device not found. Retrying...")
                # No sleep to reduce delay
            else:
                self.signals.output.emit(
                    f"Found {self.target_device_name} device: {target_device.name}, Address: {target_device.address}"
                )
                self.current_device = target_device
                service_uuid, char_uuid = await self.discover_uuids_with_retry(self.current_device)
                if service_uuid and char_uuid:
                    self.current_char_uuid = char_uuid
                    await self.handle_connection(self.current_device, service_uuid, char_uuid)
                else:
                    self.signals.output.emit(f"Failed to discover required UUIDs. Retrying scan...")

    async def discover_uuids_with_retry(self, device):
        for attempt in range(MAX_RETRIES):
            if self.stop_flag:
                return None, None
            try:
                return await self.discover_uuids(device)
            except Exception as e:
                self.signals.output.emit(f"Attempt {attempt + 1}/{MAX_RETRIES} failed: {e}")
                if attempt < MAX_RETRIES - 1:
                    self.signals.output.emit(f"Retrying...")
                    # No sleep to reduce delay
                else:
                    self.signals.output.emit("Max retries reached. Moving back to scanning.")
        return None, None

    async def discover_uuids(self, device):
    ...
    async def handle_connection(self, device, service_uuid, char_uuid):
        while not self.stop_flag:
            connected = await self.connect_and_receive(device, service_uuid, char_uuid)
            if connected:
                break
            else:
                self.signals.output.emit(f"Reconnecting...")
                # No sleep to reduce delay

    async def connect_and_receive(self, device, service_uuid, char_uuid):
        try:
            async with BleakClient(device) as client:
                def notification_handler(sender, data):
                    try:
                        ...
                        self.send_udp(data_str)  # Send data over UDP
                    except Exception as e:
                        self.signals.output.emit(f"Failed to process received data: {e}")

                await client.start_notify(char_uuid, notification_handler)
                self.signals.output.emit(f"Connected to {device.address}, receiving data...")

                while not self.stop_flag:
                    await asyncio.sleep(1)  # Keep the connection alive

                await client.stop_notify(char_uuid)
                self.signals.output.emit("Stopped notifications and disconnected.")
                return True

        except Exception as e:
            self.signals.output.emit(f"Failed to connect to {device.address}: {e}")
            return False
    ...
    def send_udp(self, message):
        try:
            ...
            self.signals.output.emit(f"Failed to send UDP message: {e}")

    @pyqtSlot()
    def send_calib(self):
        if self.current_device and self.current_char_uuid:
    ...
    async def _send_calib(self):
        try:
            async with BleakClient(self.current_device) as client:
...
class MainWindow(QMainWindow):
    ...
    def stop_connection(self):
        if self.ble_worker:
            ...
            self.ble_worker.wait()
            self.ble_worker = None
            self.textEdit.append("Disconnected")
    ...
    def display_ascii_art(self):
        ...
        for line in UNIST_DESIGN_ASCII:
